...
# Generated from help text on Tue Aug  5 16:37:25 CDT 2025

# Example 1 from lines help text
echo "🧪 Testing: ./gh-comment lines $PR_NUM src/main.go"
./gh-comment lines $PR_NUM src/main.go
if [[ $? -eq 0 ]]; then
...
echo "🧪 Testing: ./gh-comment lines $PR_NUM src/main.go --show-code"
./gh-comment lines $PR_NUM src/main.go --show-code
if [[ $? -eq 0 ]]; then
...
echo "🧪 Testing: ./gh-comment lines $PR_NUM src/main.go | grep "^42:""
./gh-comment lines $PR_NUM src/main.go | grep "^42:"
if [[ $? -eq 0 ]]; then
...
echo "🧪 Testing: ./gh-comment lines $PR_NUM src/main.go | grep -o "^[0-9]*" | head -5"
./gh-comment lines $PR_NUM src/main.go | grep -o "^[0-9]*" | head -5
if [[ $? -eq 0 ]]; then
